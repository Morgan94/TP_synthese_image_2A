...
bool cartoon=false; // just for testing/*{{{*/
vec3 getCartoonColor(const in vec3 lightcolor, const in float nDotL)
{ 
    vec3 color=lightcolor;
    float A=0.1, B=0.3, C=0.6, D=1.;
    if (nDotL > C)      color = D * lightcolor;
    else if (nDotL > B)  color = C * lightcolor;
    else if (nDotL > A)  color = B * lightcolor;
    else                   color = 0. * lightcolor;
    return color;
}/*}}}*/
